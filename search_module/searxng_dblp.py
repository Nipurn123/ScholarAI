...
def search_searxng_dblp(query: str, opts: Optional[SearxngSearchOptions] = None):
    searxng_url = "http://localhost:4000"
    url = f"{searxng_url}/search"
    
    params = {
        'q': query + ' site:dblp.org',  # Add site:dblp.org to the query
        'format': 'json'
    }
    ...
    # Filter results to only include those from dblp.org
    results = [SearxngSearchResult(**result) for result in data['results']
               if result.get('url', '').startswith('https://dblp.org')]
